...
flattenStates[basisMixon_] := Block[
   {kets, projection},
   kets = 
...
(*The purpose of the following is to undo the nested structure of the 
stored matrix element and give a single big matrix where the states 
are ordered in the same way as the list flattendStates.*)
...
sparseArrayFlatten[mat_] := Block[
   {dims = Map[Dimensions, mat//takeNonEmpty, {2}],
    rows, cols, res},
