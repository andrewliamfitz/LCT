...
monomialScalarContraction[m_][kCount_,kpCount_]:=monomialScalarContraction[m][kCount,kpCount]=Block[
	...
	Switch[
		...
		(* If d=0, the Subscript[k, i] and Subscript[k, j]' are fixed, and we can compute the resulting matrix element *)
		0,
		cBar=Ramp[diff];
		cpBar=Ramp[-diff];
...
(* 
	...
	with the mass term using monomialScalarContraction[].
...
(* 
	Suppose 'c' is a bin count of some vector vecK, cfBinReconstruct[c]
	reverses the bin count and reconstructs vecK.

	The vector is built up so that for each ki\[Element]vecK (with c[[ki]]>0) the
	reconstructed vecK contains exactly c[[ki]] copies of ki. 

	The reconstructed vecK has fixed order: the smaller ki first and larger
	ki later.
